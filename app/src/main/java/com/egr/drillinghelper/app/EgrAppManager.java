 ...
 * @author liminghuang
 */
public final class EgrAppManager {

    // 使用弱引用是因为存在未使用AppManager的finish方法来释放的activity，但mActivityStack并未断开对其应用导致内存泄露的问题
    private Stack<WeakReference<Activity>> mActivityStack;
    private static volatile EgrAppManager sInstance;

    private EgrAppManager() {
    }
     ...
     */
    public static EgrAppManager getInstance() {
        if (sInstance == null) {
            synchronized (EgrAppManager.class) {
                if (sInstance == null) {
                    sInstance = new EgrAppManager();
                }
            }
        }
        return sInstance;
